...
class RuntimeFactory:
    ...
    def create_planner(self) -> MultiAgentTripPlanner:
        """按 Run 构造编排器：LLM 与高德工具均为本次运行的独立组合。"""
        return MultiAgentTripPlanner(llm=self.create_llm(), amap_tool=self.create_amap_tool())
...
class AppRuntime:
    ...
    def __init__(
        ...
        self.runner = runner if runner is not None else RunRunner(factory=factory, registry=self.registry)
